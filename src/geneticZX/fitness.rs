...
fn get_component_complex_gates(graph: &Graph, circuit: &Circuit) -> i64 {
    ...
    let two_qubit_diff = stats.twoq as i64 - GOAL_CIRCUIT_STATS.twoq as i64;
    return two_qubit_diff;
}
...
pub fn set_fitness_values(population: &mut GraphPopulation) {
    for i in 0..population.graphs.len() {
        population.fitness_values[i] = get_fitness(&population.graphs[i], &population.circuits[i]);
    }
}
